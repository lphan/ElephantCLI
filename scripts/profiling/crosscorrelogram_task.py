...
def crosscorrelogram_task(inputdata, number_of_jobs, job_id):
    '''
        ...
        Description: This task calculates all pair-wise cross-correlograms
        between all combinations of spike trains in the input file.
        Significance of the correlation is evaluated based on spike-dither
        surrogates.
        Categories:
            - FDAT
    ...
    # print task_starts_idx
    # print task_starts_idx[job_id]
    task_stop_idx = idx[1:]
    ...
    for calc_i in range(task_starts_idx[job_id], task_stop_idx[job_id]):
        ...
        # original CCH
        cco = elephant.spikecorr.cch(sts[ni], sts[nj], w=lag_res, lag=max_lag,
                                     smooth=smoothing)
        ...
        for surrogate in range(num_surrs):
            scc = elephant.spikecorr.cch(surr_i[surrogate], surr_j[surrogate],
                                         w=lag_res, lag=max_lag,
                                         smooth=smoothing)
...
if __name__ == '__main__':
    ...
    # if number_of_jobs =2 -> it has 2 job_id, job_id should be 0, 1
    number_of_jobs = 1

    # INPUT-third parameter
    # get job parameter: a number between 0 and number_of_jobs-1
    import os
    ...
    # job_id = 1 causes ERROR index 1 is out of bounds for axis 0 with size 1
    # FIXED: job_id = number_of_jobs - 1
